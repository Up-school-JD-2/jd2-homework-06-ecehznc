...
public class ProductManager {
  ...
  public List<Product> getActiveProductsSortedByPrice() {
	  Predicate<Product> isActive = product -> product.getProductStatus() == ProductStatus.ACTIVE;
	  return products.values().stream().filter(isActive).sorted(Comparator.comparing(Product::getPrice)).collect(Collectors.toList());
	}
  
  public double calculateAveragePriceInCategory(String category) {
	  OptionalDouble averagePrice = products.values().stream()
	    .filter(product -> product.getCategory().equals(category)).mapToDouble(Product::getPrice)
	    .average();
